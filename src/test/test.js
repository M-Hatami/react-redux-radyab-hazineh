...
describe("", function () {
  it("", function () {
    ...
    try {
      ...
      expect(
        true,
        "Try checking your code again. You likely have a syntax error." + e
      ).to.equal(false);
    }
    ...
    expect(callsMethod, `Make sure to call \`createSlice()\`.`).to.not.be.null;
    ...
    try {
      ...
      expect(
        true,
        `Did you define a \`${learnerVariableName}\` variable?`
      ).to.equal(false);
    }
